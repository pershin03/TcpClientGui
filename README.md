...
Клиент для отправки запросов серверу с БД. Ждет подключения к серверу для работы. Написан с помощью QWidget. В качестве протокола обмена выбран текстовый формат JSON. Работа с сетью организована стандартными средствами Qt(общение идет через QTcpSocket).
